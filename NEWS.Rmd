...
While the first version of IDBac followed an OK paradigm for small Shiny apps (two files: server.r and ui.r), it became quickly apparent that it was an unsustainable and unscalable model.

 New paradigms:
 - IDBac is an R package
   - Now IDBac is structured as a regular R-package. This means that significant computation and code chunks have been pulled out of server.r and are now located in the packages `R` directory. 
 - IDBac is testable
   - While testing the interactivity of Shiny apps is still a little shaky, ~~all~~ most functions within the `R` directory should have a `testthat` test.
