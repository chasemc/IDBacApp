...
Create a mirror plot for samples "A" and "D". `SNR` and `peakPercentPresence` settings don't matter in this example because there isn't SNR info or replicate data. 
```{r}
...
Display a mirror plot using plotly.js:
```{r}
IDBacApp::mirrorPlot(mirror_env)
```
