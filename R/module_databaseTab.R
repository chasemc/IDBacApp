...
databaseTabUIFunc <- function(ns) {
  tagList(
    ...
    column(
      ...
      fluidRow(
        bsCollapse(
          id = ns("collapseSQLInstructions"),
          open = "Panel 1",
          bsCollapsePanel(
            h4("Open\\Close Instructions",
              align = "center"
            ),
      ...
      fluidRow(
        bsCollapse(
          id = ns("modifySqlCollapse"),
          bsCollapsePanel(
            h4("Click here to modify the selected experiment", align = "center"),
            tabsetPanel(
              id = ns("ExperimentNav"),
      ...
      fluidRow(
        bsCollapse(
          id = ns("createDbFromDb"),
          bsCollapsePanel(
            h4("Click here to copy samples from an existing experiment to a new experiment",
              align = "center"
            ),
            wellPanel(
      ...
      fluidRow(
        bsCollapse(
          id = ns("exportmzml"),
          bsCollapsePanel(
            h4("Click here to export samples as mzML",
              align = "center"
            ),
            wellPanel(
