...
mirrorPlot <- function(mirrorPlotEnv) {
  top <- cbind.data.frame(
    x = mirrorPlotEnv$spectrumSampleOne@mass,
  ...
  lp <- lapply(
    seq_along(mirrorPlotEnv$SampleOneColors),
    function(x) {
      list(
        type = "rect",
  ...
  bottom <- cbind.data.frame(
    x = mirrorPlotEnv$spectrumSampleTwo@mass,
  ...
  lp2 <- lapply(
    seq_along(mirrorPlotEnv$SampleTwoSNR),
    function(x) {
      list(
        type = "rect",
        fillcolor = "grey",
  ...
  plotly::plot_ly(
    ...
    text = paste(
      ...
      "<br> Sample:",
      mirrorPlotEnv$sampleIDOne
    ),
    hoverinfo = "text",
    line = list(color = "#000000"),
    name = mirrorPlotEnv$sampleIDOne
  ) %>%
    plotly::add_lines(
      data = bottom,
      x = ~x,
      y = ~ -y,
      line = list(color = "grey"),
      name = mirrorPlotEnv$sampleIDTwo,
      text = paste(
        "m/z:", round(bottom$x, 3),
        "<br> Intensity:", round(bottom$y, 1),
        "<br> Sample:",
        mirrorPlotEnv$sampleIDTwo
      ),
      hoverinfo = "text"
    ) %>%
    plotly::layout(
...
baserMirrorPlot <- function(mirrorPlotEnv) {
  ...
  graphics::plot(
    ...
    y = mirrorPlotEnv$spectrumSampleOne@intensity,
    ylim = c(
      -max(mirrorPlotEnv$spectrumSampleTwo@intensity),
  ...
  graphics::rect(
    xleft = mirrorPlotEnv$peaksSampleOne@mass - 0.5,
    ybottom = 0,
  ...
  graphics::rect(
    xleft = mirrorPlotEnv$peaksSampleTwo@mass - 0.5,
    ybottom = 0,
