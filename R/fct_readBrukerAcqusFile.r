...
readAcqusFile <- function(fidFile, verbose = FALSE) {
  ...
  metadata$hpcLimits <-
    c(
      min_mass = .grepAcquDoubleValue("##\\$HPClBLo=", acquLines),
  ...
  ## obligate LIFT
  metadata$lift <- c(
    .grepAcquDoubleValue("##\\$Lift1=", acquLines),
  ...
  if (length(metadata$laser_shots) == 0L) {
    warning(
      "File ", sQuote(fidFile), " seems to be empty because ",
  ...
  if (length(metadata$patch) &&
    grepl(
      pattern = "(R[0-9]+)?X[0-9]+Y[0-9]+", x = metadata$patch,
      ignore.case = TRUE
    )) {
...
.sampleName <- function(fidFile) {
  ...
  if (isShortPath) {
    ...
    sampleName <- gsub(
      pattern = "[[:punct:]]|[[:space:]]", replacement = "_",
      x = sampleName
    )
  }
...
extractHPCConstants <- function(hpcStr) {
  ...
  list(
    coefficients = as.double(tmpLine[(which(tmpLine == "V1.0VectorDouble") + 2L):
    (which(tmpLine == "c2") - 1L)]),
