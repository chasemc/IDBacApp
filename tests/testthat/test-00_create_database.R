...
protein_masses <- list(
  ...
  c(
    3091, 3951, 4070, 4772, 4821, 4846, 5090, 6488, 6493, 6675, 6962, 7187, 7252, 7655, 7672, 8034, 8913, 9030, 9469, 10030, 10071,
...
protein_spectra <- createFuzzyVector(
  massStart = 2000,
  massEnd = 30000,
  ppm = 8000,
...
small_masses <- list(
  ...
  c(
    244, 297, 302, 308, 315, 334, 343, 346, 389, 401, 427, 468, 551, 565, 610, 675, 696, 728, 738, 760, 791, 840, 841,
...
smallmol_spectra <- createFuzzyVector(
  massStart = 200,
  massEnd = 3000,
  ppm = 100,
...
smallmol_spectra <- lapply(
  1:ncol(smallmol_spectra),
  function(x) {
    MALDIquant::createMassSpectrum(
      mass = as.numeric(rownames(smallmol_spectra)),
      intensity = smallmol_spectra[, x]
    )
  }
)
protein_spectra <- lapply(
  1:ncol(protein_spectra),
  function(x) {
    MALDIquant::createMassSpectrum(
      mass = as.numeric(rownames(protein_spectra)),
...
new_idbac_mzml <- IDBacApp::idbac_connect(
  "sql_from_mzml",
  sql_dir
)[[1]]
test_that("IDBac sql from mzml", {
  expect_message(
    db_from_mzml(
      mzFilePaths = list.files(mzml_path, full.names = T),
...
for (i in seq_along(smallmol_spectra)) {
  MALDIquantForeign::exportCsv(
    smallmol_spectra[[i]],
    file.path(
      csv_path,
      "small",
      paste0(
        names(smallmol_spectra)[[i]],
        ".csv"
      )
    )
  )
...
dir.create(new_mzml)
delim_tomzml_results <- IDBacApp::delim_to_mzml(
  proteinPaths = p_files,
...
test_that("delim_to_mzml()", {
  ...
  expect_equal(
    unique(tools::file_ext(delim_tomzml_results)),
    "mzml"
  )
})
...
new_idbac <- IDBacApp::idbac_connect(
  "sql_from_csv",
  sql_dir
)[[1]]
test_that("IDBac sql from csv", {
  expect_message(
    IDBacApp::db_from_mzml(
      mzFilePaths = unname(delim_tomzml_results),
...
samps <- idbac_available_samples(
  pool = new_idbac,
  type = "all"
)
smallspec <- idbac_get_spectra(
  pool = new_idbac,
  sampleID = LETTERS[1:4],
  type = "small"
)
protspec <- idbac_get_spectra(
  pool = new_idbac,
  sampleID = LETTERS[1:4],
  type = "protein"
)
