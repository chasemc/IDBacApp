    ...
    if (is.null(input$rawORreanalyze)){}else if (input$rawORreanalyze == 1){
      output$ui1<-renderUI({
        ...
        column(5,style = "background-color:#F5F5F5",
               ...
               fluidRow(column(
                 12,
                 verbatimTextOutput("selectedWorkingDirectory", placeholder = TRUE)
               )),
               br(),
               p(
                 strong("2:"),
                 "Your RAW data should be one folder that contains: two folders containing protein and small-molecule data"
               ),
               actionButton("rawFileDirectory", label = "Click to select the location of your RAW data"),
               fluidRow(column(
                 12, verbatimTextOutput("rawFileDirectory", placeholder = TRUE)
               )),
               br(),
               p(
                 strong("3:"),
                 "Choose  your Sample Map file, the excel sheet which IDBac will use to rename your files"
               ),
               fileInput('excelFile', label = NULL , accept =
                           c('.xlsx')),
      ...
      output$ui1<-renderUI({
        ...
        column(12,
               ...
               column(5,
                      ...
                      br(),
                      p("Your working directory is the folder which contains \"Converted_To_mzML\",\"Peak_ListsL\", and \"Saved_MANs\"       "),
...
if(input$idbacDirectoryButton==0){
  "No Folder Selected"
